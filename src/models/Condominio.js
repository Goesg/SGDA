...
class Condominio{
    ...
    async findAllByCidade(cidade){
        try{
           let user = await database.select().table(this.table).where({cidade:cidade})
           if(user.length > 0) return {status:200, result:user} 
           else return {status:404, result:{erro:`A cidade ${cidade} não corresponde a nenhum ${this.name}`}}
        }catch(err){
            console.log(err)
            throw new Error(`Erro no model ${this.name}, método findAllByCidade`)
        };
    };
...
module.exports = new Condominio('condominio','condominio')