...
class checkEmail{
    static  async checkerEmail(email){
        try{
            ...
            let existEmail = await database.select().from('condominios').where({email:email}).then(result =>{
...
class checkCpf{
    static  async checkerCpf(cpf){
        try{
            ...
            let existCpf = await database.select().from('condominios').where({cpf:cpf}).then(result =>{
                if(result.length > 0) return true
                else{return false}                     
            });
